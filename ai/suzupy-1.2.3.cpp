...
class Suzupy : public AI {
    ...
    int Evaluate(const River& river) {
        static constexpr int kRule1Score = 200000000;
        static constexpr int kRule2Score = 100000000;

        const SiteId source = river.source;
        const SiteId target = river.target;

        // #1-a: λ鉱脈同士（またはλ鉱脈と連結成分）を接続
        // #1-b: 他人のそれを妨害
